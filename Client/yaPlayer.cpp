...
namespace ya
{
	...
	void Player::Move(int _dir)
	{
		...
		if (_dir == 0 && Input::GetKeyUp(eKeyCode::W))
		{
			...
			{
				int PlayerY = int(pos.y - chpt7->LeftTop.y) / MOVE_TILE_HEIGHT;
				int PlayerX = int(pos.x - chpt7->LeftTop.x) / MOVE_TILE_WIDTH;

				switch (chpt7->MapInfo7[PlayerY - 1][PlayerX])
				{
				case 0:
					//캐릭터 이동시키고 맵정보 2개 갱신
					tr->SetPosition(Vector2(pos.x, pos.y - MOVE_TILE_HEIGHT));
					chpt7->pointerMap7[PlayerY][PlayerX] = nullptr;
					chpt7->pointerMap7[PlayerY - 1][PlayerX] = this;

					chpt7->MapInfo7[PlayerY][PlayerX] = 0;
					chpt7->MapInfo7[PlayerY - 1][PlayerX] = 2;
				...
				case 1:
					...
					return;
				case 3:
					static_cast<Monster*>(chpt7->pointerMap7[PlayerY - 1][PlayerX])->Hit(dir); /* 몬스터 내부에 공격받는 함수 만들고 */
					break;
				case 4:
					static_cast<Rock*>(chpt7->pointerMap7[PlayerY - 1][PlayerX])->RockHit(_dir);
					break;
				default:
					break;
				}
			...
			{
				int PlayerY = int(pos.y - chpt8->LeftTop.y) / MOVE_TILE_HEIGHT;
				int PlayerX = int(pos.x - chpt8->LeftTop.x) / MOVE_TILE_WIDTH;

				switch (chpt8->MapInfo8[PlayerY - 1][PlayerX])
				{
				case 0:
					//캐릭터 이동시키고 맵정보 2개 갱신
					tr->SetPosition(Vector2(pos.x, pos.y - MOVE_TILE_HEIGHT));
					chpt8->pointerMap8[PlayerY][PlayerX] = nullptr;
					chpt8->pointerMap8[PlayerY - 1][PlayerX] = this;

					chpt8->MapInfo8[PlayerY][PlayerX] = 0;
					chpt8->MapInfo8[PlayerY - 1][PlayerX] = 2;
				...
				case 1:
					...
					return;
				case 3:
					static_cast<Monster*>(chpt8->pointerMap8[PlayerY - 1][PlayerX])->Hit(dir); /* 몬스터 내부에 공격받는 함수 만들고 */
					break;
				case 4:
					static_cast<Rock*>(chpt8->pointerMap8[PlayerY - 1][PlayerX])->RockHit(_dir);
					break;
				default:
					break;
				}
		...
		else if (_dir == 2 && Input::GetKeyUp(eKeyCode::A))
		{
			...
			if (Chapter7* chpt7 = dynamic_cast<Chapter7*>(scene))
			{
				...
				switch (chpt7->MapInfo7[PlayerY][PlayerX - 1])
				{
				case 0:
					//캐릭터 이동시키고 맵정보 2개 갱신
					tr->SetPosition(Vector2(pos.x - MOVE_TILE_WIDTH, pos.y));
					chpt7->pointerMap7[PlayerY][PlayerX] = nullptr;
					chpt7->pointerMap7[PlayerY][PlayerX - 1] = this;

					chpt7->MapInfo7[PlayerY][PlayerX] = 0;
					chpt7->MapInfo7[PlayerY][PlayerX - 1] = 2;

					break;
				case 1:
					return;
				case 3:
					static_cast<Monster*>(chpt7->pointerMap7[PlayerY][PlayerX - 1])->Hit(_dir) /* 몬스터 내부에 공격받는 함수 만들고 */;
					break;
				case 4:
					static_cast<Rock*>(chpt7->pointerMap7[PlayerY][PlayerX - 1])->RockHit(_dir);
					break;
				default:
					break;
				}
			...
			if (Chapter8* chpt8 = dynamic_cast<Chapter8*>(scene))
			{
				...
				switch (chpt8->MapInfo8[PlayerY][PlayerX - 1])
				{
				case 0:
					//캐릭터 이동시키고 맵정보 2개 갱신
					tr->SetPosition(Vector2(pos.x - MOVE_TILE_WIDTH, pos.y));
					chpt8->pointerMap8[PlayerY][PlayerX] = nullptr;
					chpt8->pointerMap8[PlayerY][PlayerX - 1] = this;

					chpt8->MapInfo8[PlayerY][PlayerX] = 0;
					chpt8->MapInfo8[PlayerY][PlayerX - 1] = 2;

					break;
				case 1:
					return;
				case 3:
					static_cast<Monster*>(chpt8->pointerMap8[PlayerY][PlayerX - 1])->Hit(_dir) /* 몬스터 내부에 공격받는 함수 만들고 */;
					break;
				case 4:
					static_cast<Rock*>(chpt8->pointerMap8[PlayerY][PlayerX - 1])->RockHit(_dir);
					break;
				default:
					break;
				}
		...
		else if (_dir == 1 && Input::GetKeyUp(eKeyCode::S))
		{
			...
			if (Chapter7* chpt7 = dynamic_cast<Chapter7*>(scene))
			{
				...
				case 1:
					return;
				case 3:
					static_cast<Monster*>(chpt7->pointerMap7[PlayerY + 1][PlayerX])->Hit(_dir) /* 몬스터 내부에 공격받는 함수 만들고 */;
					break;
				case 4:
					static_cast<Rock*>(chpt7->pointerMap7[PlayerY + 1][PlayerX])->RockHit(_dir);
					break;
			...
			if (Chapter8* chpt8 = dynamic_cast<Chapter8*>(scene))
			{
				...
				case 1:
					return;
				case 3:
					static_cast<Monster*>(chpt8->pointerMap8[PlayerY + 1][PlayerX])->Hit(_dir) /* 몬스터 내부에 공격받는 함수 만들고 */;
					break;
				case 4:
					static_cast<Rock*>(chpt8->pointerMap8[PlayerY + 1][PlayerX])->RockHit(_dir);
					break;
